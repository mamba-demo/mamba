...
class SwinTransformer3d(nn.Module):
    ...
    def load_weights(self, load_weights=True, weights=None, **kwargs):
        if load_weights:
            print("Loading pretrained video weights")
            if self.name == 'small':
                weights = Swin3D_S_Weights.verify(Swin3D_S_Weights.KINETICS400_V1)
            elif self.name == 'base':
                weights = Swin3D_B_Weights.verify(Swin3D_B_Weights.KINETICS400_IMAGENET22K_V1)
            else:
                raise NotImplementedError("No pretrained weights for ", self.name)

            self.load_state_dict(weights.get_state_dict(progress=False))
        else:
