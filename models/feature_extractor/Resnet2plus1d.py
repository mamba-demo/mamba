    ...
    def load_weights(self, load_weights=True, pretrained_path=None, **kwargs):
        ...
        if load_weights:
            if 'imagenet' in pretrained_path:
                print("Loading pretrained r2plus1d weights")

                model_name = "r2plus1d_r50"
                loaded_model = torch.hub.load("facebookresearch/pytorchvideo", model=model_name, pretrained=True)

                self.load_state_dict(loaded_model.state_dict(), strict=False)
            else:
                raise NotImplementedError("Not implemented for {}".format(pretrained_path))
...
def create_r2plus1d(
    *,
    ...
    _MODEL_STAGE_DEPTH = {50: (3, 4, 6), 101: (3, 4, 23), 152: (3, 8, 36)}
    ...
    # )
    # blocks.append(head)
    return CustomNet(blocks=nn.ModuleList(blocks))
